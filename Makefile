...
.PHONY: test-docker
test-docker:
	$(DOCKER) run --rm -e PYTHONPATH=/app app uv run pytest app/tests --cov=app --cov-report=term-missing

.PHONY: test-docker-cov
test-docker-cov:
	$(DOCKER) run --rm -e PYTHONPATH=/app -v $(PWD)/htmlcov:/app/htmlcov app uv run pytest app/tests --cov=app --cov-report=html
	@echo "Coverage report → htmlcov/index.html"# =========================================
# Security
# =========================================
.PHONY: bandit
